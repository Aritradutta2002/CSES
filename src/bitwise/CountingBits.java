...
public class CountingBits {
    ...
    static void solve() throws IOException {
        ...
        for (int k = 0; (1L << k) <= n; k++) {
            long powerOfTwo = 1L << k; // 2^k
            long fullCycleLen = powerOfTwo << 1; // 2^(k+1)

            long fullGroups = (n + 1) / fullCycleLen;
            long remainder = (n + 1) % fullCycleLen;

            ans += fullGroups * powerOfTwo;
            ans += Math.max(0, remainder - powerOfTwo);
        }
